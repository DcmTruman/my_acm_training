...
int main()
{
    ...
    ww(~scanf("%d%d%d",&N,&F,&D)){
        dinc.init();
        repf(i,1,F){
            sc(x);dinc.addedge(0,i,x);
        }
        repf(i,1,D){
            sc(x);dinc.addedge(i+F,D+F+N+N+1,x);
        }
        repf(i,1,N){
           dinc.addedge(D+F+i,D+F+N+i,1);
           scanf("%s",stmp);
           rep(j,F){
               if(stmp[j] == 'Y'){
                   dinc.addedge(j+1,D+F+i,1);
               }
           } 
        }
        repf(i,1,N){
            ...
            rep(j,D){
                if(stmp[j] == 'Y'){
                    dinc.addedge(D+F+i+N,j+F+1,1);
                }
            }
        }
        pc(dinc.Maxflow(0,D+F+N+N+1));
    }
return 0;
}